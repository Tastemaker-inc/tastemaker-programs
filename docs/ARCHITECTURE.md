...
- **governance**: reads project_escrow Project and Backer; CPIs project_escrow `release_milestone` with release PDA signer.
...
- **project_escrow:** `[b"artist_state", artist]`, `[b"project", artist, project_index_le]`, `[b"escrow", project]`, `[b"backer", project, wallet]`, `[b"project", project]` (escrow authority), `[b"burn_vault"]` (fee burn).
...
- Escrow release is **only** via governance CPI; `release_milestone` requires `governance_authority.is_signer` (governance release PDA).
